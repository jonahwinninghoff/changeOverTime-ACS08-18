...
rejAdj <- lapply(alphas,function(alpha) mult3(pTrend,alpha,TRUE))
names(rejAdj) <- alphas
    ...
    anal[[nn]]$diffStars <-
        if(names(anal[[nn]])[1]=='deaf')
            rbind(unadj=rep('',ncol(anal[[nn]]$deaf[[1]])),#vapply(1:ncol(anal[[nn]]$deaf[[1]]),function(cc) stars(rejUnadj,'diff',nn,cc),'a'),
                  adj=rep('',ncol(anal[[nn]]$deaf[[1]])))#vapply(1:ncol(anal[[nn]]$deaf[[1]]),function(cc) stars(rejAdj,'diff',nn,cc),'a'))
...
subgroups of deaf Americans; <!--corrected with the Hommel procedure
